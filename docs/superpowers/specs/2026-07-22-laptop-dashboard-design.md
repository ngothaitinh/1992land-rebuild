...
- Bài viết (`app/tin-tuc/[slug]/page.tsx` — `renderBody`) giữ nguyên cách render riêng hiện có, không đụng vào ở v1 (làm đợt sau cùng lúc mở rộng dashboard cho bài viết). **Cập nhật:** việc này đã xảy ra — `renderBody` đã bị xoá, trang bài viết nay dùng chung `parseMarkdownBlocks`/`<MarkdownBlocks>` (xem `docs/superpowers/specs/2026-08-20-dashboard-shell-and-posts-design.md` §7.2).
