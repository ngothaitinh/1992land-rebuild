...
test("checkDashboardSession — /me trả 200 → 'ok'", async (t) => {
...
test("checkDashboardSession — /me trả 401 → 'unauthorized' không throw", async (t) => {
  t.mock.method(globalThis, "fetch", fakeFetch(401, { error: "Chưa đăng nhập" }));
  const result = await checkDashboardSession("https://api.example.com");
  assert.equal(result, "unauthorized");
});

test("checkDashboardSession — /me trả 502 (backend chưa lên) → 'error', KHÔNG phải 'unauthorized'", async (t) => {
  // Finding I5: lỗi backend-unreachable từng bị gộp chung với 401 thành cùng
  // một giá trị false, khiến layout luôn redirect về login dù đăng nhập lại
  // cũng không sửa được lỗi -> vòng lặp vô hạn. Test này khoá lại việc 2 trường
  // hợp phải phân biệt được nhau.
  t.mock.method(globalThis, "fetch", fakeFetch(502, { error: "Bad Gateway" }));
  const result = await checkDashboardSession("https://api.example.com");
  assert.equal(result, "error");
});

test("checkDashboardSession — fetch throw (mất mạng/DNS lỗi) → 'error'", async (t) => {
  t.mock.method(globalThis, "fetch", mock.fn(async () => { throw new TypeError("fetch failed"); }));
  const result = await checkDashboardSession("https://api.example.com");
  assert.equal(result, "error");
});
